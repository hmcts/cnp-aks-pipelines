...
do
  ...
  if [[ $repo =~ (^base/.*|^cmc/ccd-definition-importer$|^imported/.*|k8s-dns-node-cache-amd64|node-load|^rpe/auto-reply-urls$|rse/check|sscs/ccd-definition-importer-benefit) ]]; then
